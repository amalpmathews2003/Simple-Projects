...
loading_img=pg.image.load("modified_cover.png")
x_img=pg.image.load("X_modified.png")
o_img=pg.image.load("O_modified.png")

loading_img=pg.transform.scale(loading_img,(width,height+100))
x_img=pg.transform.scale(x_img,(80,80))
o_img=pg.transform.scale(o_img,(80,80))

def game_loading_window():
...
def check_win():
	...
	for row in range(0,3):
		if(board[row][0]==board[row][1]==board[row][2] and
			...
			pg.draw.line(screen,(255,0,0),
				(0,((row+1)*height/3-height/6),(width,(row+1)*height/3-height/6),4))
			break
	...
	for col in range(0,3):
		if(board[0][col]==board[1][col]==board[2][col] and
			...
			winner=board[0][col]
			pg.draw.line(screen,(255,0,0),
				(0,(col+1*width/3-width/6),(height,(col+1)*width/3-width/6),4))
			break
